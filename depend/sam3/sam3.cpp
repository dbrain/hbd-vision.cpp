...
struct sam3_model {
    ...
    // Upstream declared sam3_free_model() but never called it, and this struct
    // had no destructor — so every load leaked its CUDA backend, weight buffer
    // (1088 MiB at q8_0) and ggml context. It is invisible in a load-once CLI,
    // but it makes evicting the model reclaim nothing, which is the whole point
    // of eviction in a long-lived multi-model server. Defined out-of-line below
    // sam3_free_model; that function nulls what it frees, so an explicit call
    // followed by destruction stays safe.
    ~sam3_model();
...
struct sam3_state {
    ...
    // Regenerated RoPE tables for the global-attention ViT blocks when encoding
    // at a non-native grid. The windowed blocks are unaffected — their table is
    // 24x24 regardless of the image grid. Empty when running native.
    struct ggml_context*  rope_ctx = nullptr;
    ggml_backend_buffer_t rope_buf = nullptr;
    std::vector<struct ggml_tensor*> rope_global;  // one per ViT block, null if windowed
...
// Spatial grid implied by a flattened [D, N_spatial, B] feature tensor. Graph
// builders that only see the features must not assume the native 72x72 grid —
// encoding at a reduced resolution changes it.
static int sam3_grid_from_spatial(int64_t n_spatial) {
    int g = (int)(std::sqrt((double)n_spatial) + 0.5);
    return ((int64_t)g * g == n_spatial) ? g : 0;
}
...
**
** The graph builders below are pure ggml_context functions: none of them is
** handed a device. That is fine for every op with a universal kernel, but
** flash-attention is implemented per head dimension and an unsupported one is
** a hard GGML_ABORT inside the backend, not a fallback -- sam3 drives a single
** backend through ggml_gallocr with no ggml_backend_sched to catch it. Rather
** than thread a device handle through eleven layers of builders to answer one
** question, the active backend is parked here by every public entry point that
** builds a graph. Null (no model loaded yet) means "assume supported", which
** keeps the CPU-only path byte-identical to before this existed.
*/
static ggml_backend_t g_sam3_active_backend = nullptr;

static bool sam3_backend_supports(struct ggml_tensor* op) {
    return !g_sam3_active_backend || ggml_backend_supports_op(g_sam3_active_backend, op);
}

/*
** SAM3_NO_FLASH_ATTN=1 forces the explicit-scores attention below on a device
** that does have flash-attention kernels. That is the only way to run the graph
** CUDA is forced to run on hardware that isn't CUDA -- CPU, Metal and Vulkan all
** accept the head dims CUDA refuses, so without it the fallback never executes
** anywhere it can be checked against a reference.
*/
static bool sam3_flash_attn_disabled() {
    static const bool off = [] {
        const char* v = getenv("SAM3_NO_FLASH_ATTN");
        return v && *v && *v != '0';
    }();
    return off;
}
...
/*
** Attention with a backend-chosen implementation. Same arguments and same
** [HD, NH, N_q, B] output as ggml_flash_attn_ext; falls back to an explicit
** scores matrix when the device has no kernel for this head dim. CUDA has none
** at 32, which is what neck_dim/8-heads gives the fusion encoder, the DETR
** decoder and the SAM mask decoder.
**
** An F32 mask also takes the fallback: flash_attn_ext only stores masks as F16,
** and the box-relative positional bias loses too much precision that way.
*/
static struct ggml_tensor* sam3_attn_ext(struct ggml_context* ctx,
                                         struct ggml_tensor* q,
                                         struct ggml_tensor* k,
                                         struct ggml_tensor* v,
                                         struct ggml_tensor* mask,
                                         float scale) {
    if ((!mask || mask->type == GGML_TYPE_F16) && !sam3_flash_attn_disabled()) {
        struct ggml_tensor* fa = ggml_flash_attn_ext(ctx, q, k, v, mask, scale, 0.0f, 0.0f);
        if (sam3_backend_supports(fa)) {
            return fa;
        }
    }
    auto* kq = ggml_mul_mat(ctx, k, q);                    // [N_kv, N_q, NH, B]
    kq = ggml_soft_max_ext(ctx, kq, mask, scale, 0.0f);
    auto* vt = ggml_cont(ctx, ggml_transpose(ctx, v));     // [N_kv, HD, NH, B]
    auto* out = ggml_mul_mat(ctx, vt, kq);                 // [HD, N_q, NH, B]
...
** ── Window partition ─────────────────────────────────────────────────────
**
** ggml's WIN_PART/WIN_UNPART have a CPU kernel and nothing else, so the ViT's
** windowed attention blocks would knock the whole encoder off any GPU backend.
** Both are pure data movement and need no new kernel: the embedding dim and the
** x-coordinate inside a window are adjacent and contiguous in the source, so
** merging them into one view dimension leaves exactly four dims and a single
** strided view + cont expresses the shuffle.
*/
static struct ggml_tensor* sam3_win_part(struct ggml_context* ctx,
    ...
    const int64_t npx = (W + w - 1) / w;
    const int64_t npy = (H + w - 1) / w;

    struct ggml_tensor* pad = a;
    if (npx * w != W || npy * w != H) {
        pad = ggml_pad(ctx, a, 0, (int)(npx * w - W), (int)(npy * w - H), 0);
    }

    const size_t ts = ggml_type_size(GGML_TYPE_F32);
    // [E*w, npx, w, npy] = (embed + x-in-window, window-x, y-in-window, window-y)
    auto* v = ggml_view_4d(ctx, pad, E * w, npx, w, npy,
                           E * w * ts,
                           E * npx * w * ts,
                           E * npx * w * w * ts,
                           0);
    auto* c = ggml_cont(ctx, ggml_permute(ctx, v, 0, 2, 1, 3));
    return ggml_reshape_4d(ctx, c, E, w, w, npx * npy);
}

static struct ggml_tensor* sam3_win_unpart(struct ggml_context* ctx,
                                           struct ggml_tensor* a,
                                           int64_t w0,
                                           int64_t h0,
                                           int w) {
    ...
    const int64_t E   = a->ne[0];
    const int64_t npx = (w0 + w - 1) / w;
    const int64_t npy = (h0 + w - 1) / w;
    GGML_ASSERT(a->ne[1] == w && a->ne[2] == w && a->ne[3] == npx * npy);

    const size_t ts = ggml_type_size(GGML_TYPE_F32);
    auto* v = ggml_view_4d(ctx, a, E * w, npx, w, npy,
                           E * w * w * ts,
                           E * w * ts,
                           E * w * w * npx * ts,
                           0);
    auto* c = ggml_reshape_4d(ctx, ggml_cont(ctx, v), E, npx * w, npy * w, 1);
    if (npx * w == w0 && npy * w == h0) {
        return c;
    }
    return ggml_cont(ctx, ggml_view_4d(ctx, c, E, w0, h0, 1,
...
// 3x3 stride-1 same-padding conv without the im2col staging buffer.
// ggml_conv_2d_s1_ph goes through im2col, which at the neck's scale-0 output
// (288x288x256) materialises a [3*3*256, 288*288] F16 tensor — ~382 MiB, the
// largest single thing in the encode arena. GGML_OP_CONV_2D has a CUDA kernel
// and computes the same result directly.
// Measured on a 3060: the direct kernel is the slower of the two per-op (the
// im2col path feeds a batched GEMM), so it is worth taking only where the
// staging buffer actually drives the arena. At the neck's scale 0 that buffer is
// ~382 MiB and sets the peak; at scale 2/3 it is 24/6 MiB and the peak is set
// elsewhere, so paying the direct kernel there is pure loss. Threshold in MiB,
// overridable — 0 disables the direct path entirely.
static struct ggml_tensor* sam3_conv_2d_3x3_s1(struct ggml_context* ctx,
                                               struct ggml_tensor* w,
                                               struct ggml_tensor* x) {
    if (getenv("SAM3_LEGACY_CONV2D") || w->ne[0] != 3 || w->ne[1] != 3) {
        return ggml_conv_2d_s1_ph(ctx, w, x);
    }
    // What ggml_im2col would stage: [kW*kH*Cin, W*H] in F16.
    const double im2col_mib = double(w->ne[0]) * double(w->ne[1]) * double(w->ne[2]) *
                              double(x->ne[0]) * double(x->ne[1]) * 2.0 / (1024.0 * 1024.0);
    double budget = 128.0;
    if (const char* env = getenv("SAM3_IM2COL_BUDGET_MIB")) {
        budget = atof(env);
    }
    if (budget <= 0.0 || im2col_mib <= budget) {
        return ggml_conv_2d_s1_ph(ctx, w, x);
    }
    return ggml_conv_2d_direct(ctx, w, x, 1, 1, 1, 1, 1, 1);
}

// ── RoPE table generation for a non-native encode grid ───────────────────
//
// SAM 3's ViT RoPE is axial, head_dim 64 -> 16 frequency pairs per axis, with
// theta 10000. The positions are NOT absolute: they are normalised to the
// 24x24 pretrained grid, i.e. t = i * 24 / G. That is why a windowed block's
// 24x24 table and a global block's 72x72 table disagree at the same (y, x) —
// the global one is compressed by 24/72.
//
// Verified against the shipped weights: this reproduces both the 24x24 window
// table and the 72x72 global table to float32 round-off (max |diff| 1.3e-6).
// Layout matches the file: [2, 32, N] with (cos, sin) fastest, pairs 0-15 = x,
// 16-31 = y, token index n = y*G + x.
static void sam3_gen_rope_table(int grid, int pretrained_grid, int head_dim,
                                std::vector<float>& out) {
    const int half = head_dim / 2;      // 32 pairs
    const int per_axis = half / 2;      // 16 per axis
    const int64_t n = (int64_t)grid * grid;
    out.resize((size_t)n * half * 2);
    const double scale = (double)pretrained_grid / (double)grid;
    for (int64_t idx = 0; idx < n; ++idx) {
        const double tx = (double)(idx % grid) * scale;
        const double ty = (double)(idx / grid) * scale;
        for (int k = 0; k < per_axis; ++k) {
            const double f = 1.0 / std::pow(10000.0, (double)(4 * k) / (double)head_dim);
            const size_t bx = ((size_t)idx * half + k) * 2;
            out[bx + 0] = (float)std::cos(tx * f);
            out[bx + 1] = (float)std::sin(tx * f);
            const size_t by = ((size_t)idx * half + per_axis + k) * 2;
            out[by + 0] = (float)std::cos(ty * f);
            out[by + 1] = (float)std::sin(ty * f);
        }
    }
}
...
// Transposed convolution with stride == kernel, as a matmul plus a pixel
// shuffle. Every deconv in SAM 3 is k=2 s=2 p=0 (both neck scales, both mask
// decoder upsamples), so the kernel taps never overlap in the output and the op
// is exactly: one GEMM over Cin, then interleave the kW x kH taps into the
// spatial dims.
//
// This exists because `ggml_conv_transpose_2d_p0` is the single most expensive
// thing in the encode. Measured with nsys on a 3060, `conv2d_transpose_kernel`
// was **77.8% of all GPU time** — 5.03 s of 6.46 s over 12 invocations, ~419 ms
// each — because the CUDA kernel is a direct gather that reduces over Cin per
// output element with no tiling or tensor-core use. The same arithmetic as a
// GEMM is ~2 orders of magnitude faster.
//
// Falls back to the stock op when stride != kernel, where taps do overlap and
// this factorisation does not hold.
static struct ggml_tensor* sam3_conv_transpose_2d_sk(struct ggml_context* ctx,
                                                     struct ggml_tensor* w,
                                                     struct ggml_tensor* x,
                                                     int stride) {
    ...
    const int64_t Cin = w->ne[3];
    const int64_t W = x->ne[0];
    const int64_t H = x->ne[1];
    const int64_t B = x->ne[3];

    if (kW != stride || kH != stride || x->ne[2] != Cin || B != 1 ||
        getenv("SAM3_LEGACY_DECONV")) {
        return ggml_conv_transpose_2d_p0(ctx, w, x, stride);
    }

    // [kW, kH, Cout, Cin] -> [Cin, kW*kH*Cout], so Cin is the reduction dim.
    auto* w2 = ggml_cont(ctx, ggml_permute(ctx, w, 1, 2, 3, 0));
    w2 = ggml_reshape_2d(ctx, w2, Cin, kW * kH * Cout);

    // [W, H, Cin, 1] -> [Cin, W*H]
    auto* x2 = ggml_cont(ctx, ggml_permute(ctx, x, 1, 2, 0, 3));
    x2 = ggml_reshape_2d(ctx, x2, Cin, W * H);

    // [kW*kH*Cout, W*H]: row index is kw + kh*kW + oc*kW*kH, col is ix + iy*W.
    auto* r = ggml_mul_mat(ctx, w2, x2);

    // Interleave the taps with the spatial dims. ggml is 4D, so this takes two
    // passes: first bring ix next to kw, then iy next to kh.
    r = ggml_reshape_4d(ctx, r, kW, kH * Cout, W, H);
    r = ggml_cont(ctx, ggml_permute(ctx, r, 0, 2, 1, 3));  // [kw, ix, (kh,oc), iy]
    r = ggml_reshape_4d(ctx, r, kW * W, kH, Cout, H);
    r = ggml_cont(ctx, ggml_permute(ctx, r, 0, 1, 3, 2));  // [ox, kh, iy, oc]
    return ggml_reshape_4d(ctx, r, kW * W, kH * H, Cout, 1);
}
...
std::shared_ptr<sam3_model> sam3_load_model(const sam3_params& params) {
    ...
    if (!model->backend && params.use_gpu) {
        // Whatever the ggml build registered — CUDA, HIP, Vulkan, SYCL — preferring
        // a discrete GPU, then an integrated one, then CPU.
    ...
    g_sam3_active_backend = model->backend;
...
void sam3_free_model(sam3_model& model) {
    ...
    if (model.backend) {
        if (g_sam3_active_backend == model.backend) {
            g_sam3_active_backend = nullptr;
        }
...
sam3_state_ptr sam3_create_state(const sam3_model& model,
    ...
    // Encoding off-native needs the global blocks' RoPE tables rebuilt for the
    // new grid (the windowed blocks are 24x24 whatever the image size, so they
    // are reused as-is), and the pretrained 24x24 pos_embed has to tile the grid
    // exactly — so the grid must be a whole multiple of 24.
    if (!hp.is_sam2() && !hp.is_edgetam() && state->encode_feat_size != hp.n_img_embd()) {
        const int grid = state->encode_feat_size;
        const int pretrained = hp.img_size / hp.patch_size / 3;  // 24
        if (grid <= 0) {
            fprintf(stderr, "%s: encode_img_size=%d gives an empty grid\n", __func__, eis);
            return nullptr;
        }
        // 24 is the window size AND the pos_embed extent. Off a multiple of it,
        // the ViT's windowed attention has to zero-pad partial windows — padding
        // the model never saw in training — and quality falls off at the right
        // and bottom edges. Allowed, because it is a real trade rather than a
        // hard failure, but it is not free and it is not the recommended ladder.
        if (grid % hp.vit_window_size != 0) {
            fprintf(stderr,
                    "%s: WARNING encode_img_size=%d gives a %dx%d grid, not a multiple of the "
                    "%d window — partial windows get zero-padded. Prefer a multiple of %d.\n",
                    __func__, eis, grid, grid, hp.vit_window_size,
                    hp.vit_window_size * hp.patch_size);
        }
        (void)pretrained;

        const int head_dim = hp.vit_head_dim();
        std::vector<float> table;
        sam3_gen_rope_table(grid, pretrained, head_dim, table);

        struct ggml_init_params ip = {
            /*.mem_size   =*/ggml_tensor_overhead() * (size_t)(hp.vit_depth + 8),
            /*.mem_buffer =*/nullptr,
            /*.no_alloc   =*/true,
        };
        state->rope_ctx = ggml_init(ip);
        state->rope_global.assign(hp.vit_depth, nullptr);
        struct ggml_tensor* shared = ggml_new_tensor_3d(
            state->rope_ctx, GGML_TYPE_F32, 2, head_dim / 2, (int64_t)grid * grid);
        ggml_set_name(shared, "vit.rope_global.resized");
        for (int i = 0; i < hp.vit_depth; ++i) {
            if (hp.is_global_attn(i)) {
                state->rope_global[i] = shared;
            }
        }
        state->rope_buf =
            ggml_backend_alloc_ctx_tensors(state->rope_ctx, model.backend);
        if (!state->rope_buf) {
            fprintf(stderr, "%s: failed to allocate resized RoPE buffer\n", __func__);
            return nullptr;
        }
        ggml_backend_tensor_set(shared, table.data(), 0, table.size() * sizeof(float));
        fprintf(stderr, "%s: rebuilt global RoPE for %dx%d grid (%zu KiB)\n",
                __func__, grid, grid, table.size() * sizeof(float) / 1024);
    }
...
void sam3_free_state(sam3_state& state) {
    ...
    if (state.rope_buf) {
        ggml_backend_buffer_free(state.rope_buf);
        state.rope_buf = nullptr;
    }
    if (state.rope_ctx) {
        ggml_free(state.rope_ctx);
        state.rope_ctx = nullptr;
    }
    state.rope_global.clear();
    if (state.ctx) {
        ggml_free(state.ctx);
        state.ctx = nullptr;
    }
...
static struct ggml_tensor* sam3_vit_block_forward(struct ggml_context* ctx,
...

    {
        ...
        auto* attn_out = sam3_attn_ext(ctx, Q, K, V, nullptr, scale);
...
static struct ggml_tensor* sam3_build_vit_prefix_graph(struct ggml_context* ctx,
    ...
    // Grid comes from the input, not hp — encoding at a reduced resolution is
    // the whole point of sam3_params::encode_img_size, and hardcoding
    // hp.n_img_embd() here made that setting assert instead of work.
    const int W = (int)x->ne[1];
    const int H = (int)x->ne[2];

    // pos_embed is [E, 24, 24] — the WINDOW size, not a global grid: it is a
    // per-window embedding tiled across windows. Tile to the next whole multiple
    // and crop, so a grid that is not a multiple of 24 still gets the correct
    // pos[y % 24, x % 24] (ggml_repeat itself only does whole multiples).
    auto* pos_2d = model.vit.pos_embed;
    const int PG = (int)pos_2d->ne[1];
    const int tw = ((W + PG - 1) / PG) * PG;
    const int th = ((H + PG - 1) / PG) * PG;
    auto* pos_target = ggml_new_tensor_4d(ctx, GGML_TYPE_F32, E, tw, th, 1);
    auto* pos_tiled = ggml_repeat(ctx, pos_2d, pos_target);
    if (tw != W || th != H) {
        pos_tiled = ggml_cont(ctx, ggml_view_4d(ctx, pos_tiled, E, W, H, 1,
                                                pos_tiled->nb[1], pos_tiled->nb[2],
                                                pos_tiled->nb[3], 0));
    }
...
static struct ggml_tensor* sam3_build_vit_graph(struct ggml_context* ctx,
                                                struct ggml_tensor* input,
                                                const sam3_model& model,
                                                const std::vector<struct ggml_tensor*>* rope_override = nullptr) {
    const auto& hp = model.hparams;
    ...
    for (int i = 0; i < hp.vit_depth; ++i) {
        sam3_vit_block blk = model.vit.blocks[i];
        if (rope_override && (size_t)i < rope_override->size() && (*rope_override)[i]) {
            blk.freqs_cis = (*rope_override)[i];
        }
        x = sam3_vit_block_forward(ctx, x, blk, hp, i);
    }
...
static void sam3_build_neck_graph(struct ggml_context* ctx,
    ...
    // Scale 0 (4× upsample)
    {
        auto* s0 = sam3_conv_transpose_2d_sk(ctx, sam3_conv_transpose_weight(ctx, neck.scales[0].deconv1_w), x, 2);
        s0 = add_bias(s0, neck.scales[0].deconv1_b);
        s0 = ggml_gelu_erf(ctx, s0);
        s0 = sam3_conv_transpose_2d_sk(ctx, sam3_conv_transpose_weight(ctx, neck.scales[0].deconv2_w), s0, 2);
        ...
        s0 = sam3_conv_2d_3x3_s1(ctx, neck.scales[0].conv3x3_w, s0);
    ...
    // Scale 1 (2× upsample)
    {
        auto* s1 = sam3_conv_transpose_2d_sk(ctx, sam3_conv_transpose_weight(ctx, neck.scales[1].deconv1_w), x, 2);
        ...
        s1 = sam3_conv_2d_3x3_s1(ctx, neck.scales[1].conv3x3_w, s1);
    ...
    // Scale 2 (1×)
    {
        ...
        s2 = sam3_conv_2d_3x3_s1(ctx, neck.scales[2].conv3x3_w, s2);
    ...
    // Scale 3 (0.5× downsample)
    {
        ...
        s3 = sam3_conv_2d_3x3_s1(ctx, neck.scales[3].conv3x3_w, s3);
...
static struct ggml_tensor* sam3_text_block_forward(struct ggml_context* ctx,
    ...
    float scale = 1.0f / sqrtf((float)HD);
    auto* attn_out = sam3_attn_ext(ctx, Q, K, V, causal_mask, scale);
...
static struct ggml_tensor* edgetam_perceiver_layer_forward(
    ...
    // ── Cross-attention: latents attend to features ─────────────────────
    {
        ...
        auto* attn = sam3_attn_ext(ctx, q, k, v, nullptr, scale);
...
bool sam3_encode_image(sam3_state& state,
    ...
    auto* vit_out = sam3_build_vit_graph(ctx0, inp, model, &state.rope_global);
    ...
    // The tracker neck is only consumed by the PVS / video-tracking path. A
    // PCS-only caller (vision-server's /parts) never reads it, and building it
    // costs a second set of 2x2 deconvs plus their 288x288x256 outputs. Opt-in,
    // because sam3_state cannot know what the caller will ask for next — set it
    // only in a process that will never track.
    const bool skip_trk_neck = getenv("SAM3_PCS_ONLY") != nullptr;
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        // Unbuilt when PCS-only: leave it null so a stray tracker call fails
        // loudly instead of reading an unallocated tensor.
        state.neck_trk[i] = skip_trk_neck ? nullptr : neck_trk_out[i];
    }
    ...
    // PEs live in a separate buffer so they survive gallocr teardown.
    {
        ...
        const int scale_sizes[4] = {
            // Follow the ENCODE grid, not the native one — these size the PE
            // tensors that pair with the neck outputs.
            sam3_eff_feat_size(state, hp) * 4,
            sam3_eff_feat_size(state, hp) * 2,
            sam3_eff_feat_size(state, hp),
            sam3_eff_feat_size(state, hp) / 2,
        };
...
static struct ggml_tensor * sam3_build_vit_attn_core_from_qkv(struct ggml_context * ctx,
    ...
    struct ggml_tensor * attn_out = sam3_attn_ext(ctx, Q, K, V, nullptr, scale);
    ...
    struct ggml_tensor* attn_mask = nullptr)  // [N_kv, N_q] or nullptr
{
    ...
    float scale = 1.0f / sqrtf((float)HD);
    auto* attn_out = sam3_attn_ext(ctx, Q, K, V, attn_mask, scale);
    ...
    struct ggml_tensor* img_pe)     // [D, N_img, 1] sinusoidal PE
{
    ...
    for (int i = 0; i < (int)ge.layers.size(); ++i) {
        ...
        // 1. Self-attention (pre-norm, pos_enc_at_attn=False)
        {
            ...
            float scale = 1.0f / sqrtf((float)HD);
            auto* sa_out = sam3_attn_ext(ctx, Q, K, V, nullptr, scale);
        ...
        // 2. Cross-attention (pre-norm, Q from x, K from img+PE, V from img)
        {
            ...
            float scale = 1.0f / sqrtf((float)HD);
            auto* ca_out = sam3_attn_ext(ctx, Q, K, V, nullptr, scale);
...
static struct ggml_tensor* sam3_fenc_layer_forward(
    ...
    // ca_q_w stores fused [D, 3*D] weights split as Q-proj, K-proj, V-proj.
    {
        ...
        float scale = 1.0f / sqrtf((float)HD);
        auto* ca_out = sam3_attn_ext(ctx, Q, K, V, ca_mask, scale);
...
static struct ggml_tensor* sam3_ddec_layer_forward(
    ...
    // 2. Cross-attention to text tokens (post-norm)
    {
        ...
        float scale = 1.0f / sqrtf((float)HD);
        auto* ca_out = sam3_attn_ext(ctx, Q, K, V, text_mask, scale);
        ca_out = ggml_reshape_3d(ctx, ca_out, D, N_q, B);
    ...
    // 3. Cross-attention to conditioned image features (post-norm)
    {
        ...
        float scale = 1.0f / sqrtf((float)HD);
        // rpb_mask is F32, so sam3_attn_ext takes its explicit-scores path and the
        // box-relative positional bias keeps full precision.
        auto* ca_out = sam3_attn_ext(ctx, Q, K, V, rpb_mask, scale);
    ...
    struct ggml_tensor* text_valid_mask)  // [T, 1, B] or nullptr
{
    ...
    // sum_rows + 1/T rather than ggml_pool_1d(AVG): POOL_1D is a CPU-only op in
    // ggml, and the mean over dim 0 is all this ever asked it for.
    auto* tp = ggml_cont(ctx, ggml_permute(ctx, text_mlp, 1, 0, 2, 3));  // [T, D, B]
    if (text_valid_mask) {
        // text_valid_mask is [T, 1, B] and carries T/n_valid on valid tokens, 0 on
        // padding, so the fixed 1/T below comes out as the mean over valid tokens.
        tp = ggml_mul(ctx, tp, text_valid_mask);
    }
    auto* pooled_t = ggml_scale(ctx, ggml_sum_rows(ctx, tp), 1.0f / (float)T);  // [1, D, B]
    struct ggml_tensor* text_pooled =
        ggml_cont(ctx, ggml_permute(ctx, pooled_t, 1, 0, 2, 3));  // [D, 1, B]
    ...
    struct ggml_tensor* text_valid_mask = nullptr)  // [T, 1, B] for scoring (or nullptr)
{
    ...
    // From the features, not hp — the encode grid may be reduced.
    const int feat_hw = sam3_grid_from_spatial(enc_feats->ne[1]);
    GGML_ASSERT(feat_hw > 0 && "ddec: encoder features are not a square grid");
...
static struct ggml_tensor* sam3_build_seg_head_graph(
    ...
    const int64_t feat_hw = sam3_grid_from_spatial(enc->ne[1]);
    GGML_ASSERT(feat_hw > 0 && "seg head: encoder features are not a square grid");
...
sam3_result sam3_segment_pcs(sam3_state& state,
    ...
    const auto& hp = model.hparams;
    const int D = hp.neck_dim;           // 256
    // The ENCODE grid, not the native one — sam3_params::encode_img_size can
    // reduce it, and every CPU-side buffer below is sized from this.
    const int H = sam3_eff_feat_size(state, hp);  // 72 native
    const int L = hp.text_ctx_len;       // 32
    const int NQ = hp.ddec_num_queries;  // 200
...
static struct ggml_tensor* sam3_sam_attention(
    ...
    // Attention
    float scale = 1.0f / sqrtf((float)HD);
    auto* out = sam3_attn_ext(ctx, Q, K, V, nullptr, scale);
    ...
    int eff_feat_size = 0)
{
    ...
    auto* up1 = sam3_conv_transpose_2d_sk(ctx, sam3_conv_transpose_weight(ctx, dec.up1_w), src_img, 2);
    ...
    auto* up2 = sam3_conv_transpose_2d_sk(ctx, sam3_conv_transpose_weight(ctx, dec.up2_w), up1, 2);
